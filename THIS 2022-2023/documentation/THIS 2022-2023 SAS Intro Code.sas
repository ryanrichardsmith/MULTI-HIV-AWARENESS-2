...
%let country  = Tanzania;    * country name;

%let survey   = this2022; * survey prefix;

%let numreps  = 277;        * the number of Jackknife replicate weights for the survey;
...
%let filepath  = C:\Desktop; * location of datasets to be analyzed;
...
/*******************************************************************************
  3. Formating variables
     ...
     ---Approach 2. Using the PHIA permanent format library
		 This example assumes that the SAS program "THIS 2022-2023 SAS Formats.sas" has been run. 
...
proc format;
	...
	value vlsf
	  ...
	  2 = "Not virally suppressed"
	 99 = "Missing";
run;

***Approach 2. Using the PHIA permanent format library; 
options fmtsearch=(phiadata.&survey.formats); * Set SAS to look for formats in the format 
                                                library created in the code above;
run;
...
set domain_gender; 
if gender = 2 /*female*/ and VarLevel = "HIV Positive"; 
run; 
...
* PHIA2 Uganda (UPHIA 2021) parameters;
