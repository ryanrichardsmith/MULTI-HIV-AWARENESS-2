...
p_load("haven","tidyverse","labelled","srvyr","lubridate","table1",
       "ggplot2","gtsummary","survey","forcats","dplyr","cardx", "viridis",
       "mice","ggmice")
...
###creating trichotomous aware/unaware/HIV negative variable
allcountries <- allcountries %>%
  mutate(statusawareness = case_when(
    tri90aware == 1 ~ "Positive: Aware",
    tri90aware == 2 ~ "Positive: Unaware",
    tri90aware == 99 ~ "Negative"
  ))

#ensuring that factors are interpreted properly
allcountries <- allcountries %>% mutate(across(where(is.labelled), to_factor))

#recoding NA values for factored variables
allcountries <- allcountries %>%
  mutate(
    ...
    hivtstrsn = fct_recode(hivtstrsn, NULL = "(-9) Refused")
  )
...
allcountries.positive <- allcountries %>%
  filter(tri90 == "(1) Aware or considered aware because ARVs detectable")

###exporting cleaned data in a .dta file to be used in Stata
write_dta(allcountries.positive, here("allcountries.dta"))