...
* Recoding and encoding variables of interst 
...
************************************
		...
		table1_mc if recentlagvl_bin == 0, by(tri90aware) ///
		vars( ///
		mi_age cat\ ///
		urban cat\ ///
		education cat\ ///
		curmar cat\ ///
		mi_timesincelasttest cat\ ///
		recentlagvl_bin cat\ ///
		) ///
		nospace onecol saving(recinfections_awareness) 
		
		table1_mc if recentlagvl_bin == 1, by(tri90aware) ///
		vars( ///
		mi_age cat\ ///
		urban cat\ ///
		education cat\ ///
		...
		nospace onecol saving(ltinfections_awareness) 
...
mi register regular age urban curmar mi_country gender_bin 
...
mi estimate, post: svy: logit awareness_outcome i.gender i.mi_region $indicontrols if mi_country == _, allbaselevels 
est store m5
...
// Only run if conducting pooled model
...
keep if mi_country == _

//dynamically creating regional passive indicators since number of regions vary
//from country to country
************************************
levelsof mi_region, local(regions)

foreach r of local regions {
    mi passive: gen mi_region`r' = (mi_region == `r')
}

levelsof mi_region, local(all_regions)

local region_indicators

local ref_region : word 1 of `all_regions'

foreach r of local all_regions {
    if "`r'" != "`ref_region'" {
        local region_indicators `region_indicators' mi_region`r'
    }
}

capture program drop fairlie_boot_all
program define fairlie_boot_all, rclass 
    fairlie awareness_outcome (mi_timesincelasttest: mi_timesincelasttest1-mi_timesincelasttest3) mi_recentlagvl1 [iweight = btwt0], by(gender_bin) pooled(mi_gender_bin0 mi_age2-mi_age7 mi_curmar2-mi_curmar5 mi_urban2 mi_education2-mi_education4 `region_indicators' mi_timesincelasttest1-mi_timesincelasttest3 mi_recentlagvl1)
